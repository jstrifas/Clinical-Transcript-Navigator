...
**`README.md` is demo-facing marketing — do NOT trust it for technical facts.** It advertises a live demo (https://clinical-transcript-navigator.vercel.app/) and a clean "4-stage" story, but several claims diverge from the actual code; trust CLAUDE.md and the source over the README. Specifically: it says "Python" (there is **zero** Python — the stack is Next.js/JS + PowerShell), "Claude Opus" (the default model is `claude-sonnet-4-6`), "13 clinical flags" (the extraction schema has **11** flag keys), and "Google Gemini independently scores across 4 dimensions" (all four evaluator dimensions are **deterministic and override Gemini** — see the Gemini evaluator section).
